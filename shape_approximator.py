...
def bSpline(anchors, order, steps):
    return bSplineBasis(order, anchors.shape[0], np.linspace(0, 1, steps)) @ anchors
...
def weighFromT(steps, t):
    p = t[:, np.newaxis] ** np.arange(steps)
...
def test_loss(new_shape, shape):
    ...
    loss = np.mean((labels - new_shape) ** 2)
    print("loss: %s" % loss)
...
def PiecewiseLinearToSpline(
    ...
    for step in progressBar(1, steps) if verbose else range(1, steps):
        ...
        m = b1 * m + (1 - b1) * grad
        v = b2 * v + (1 - b2) * grad**2
        anchors -= learnRate * m / (1 - b1**step) / (np.sqrt(v / (1 - b2**step)) + 1e-9)
    ...
    if verbose:
        points = weights @ anchors
        print(f"Final loss: {np.mean((labels - points) ** 2):3f}", end=" | ")
