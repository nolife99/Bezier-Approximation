...
class Poi:
    ...
    def rotate(self, theta):
        x = cp.cos(theta) * self.x - cp.sin(theta) * self.y
        self.y = cp.cos(theta) * self.y + cp.sin(theta) * self.x
        self.x = x
    ...
    def distance(self, point):
        return cp.sqrt((self.x - point.x) ** 2 + (self.y - point.y) ** 2)
    ...
    def getAngle(self):
        return (
            -1 * cp.arccos(self.x / self.distance(Poi(0, 0)))
            if self.y < 0
            else cp.arccos(self.x / self.distance(Poi(0, 0)))
        )
...
def modulo(a, n):
    return a - cp.floor(a / n) * n


def shortestAngleDelta(a1, a2):
    return modulo((a2 - a1 + cp.pi), (2 * cp.pi)) - cp.pi
